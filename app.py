...
def questions(id):
    ...
    for row in cur:
        ...
        #cur2.execute("SELECT author FROM comments WHERE ansid = %s",[row['id']])
        #comments_author.append(cur2.fetchall())
        #cur2.execute("SELECT comDate FROM comments WHERE ansid = %s",[row['id']])
        #comments_date.append(cur2.fetchall())
        cur2.close()
    cur.close()
    ...
    if request.method == 'POST':
        ...
        if form1.validate() :
            ...
            #print("Hello")
            #cur = mysql.connection.cursor()
            #cur.execute("SELECT upvote FROM answers WHERE ansid = %s",[answerid])
            #uvote = cur.fetchone()['upvote'];
            #uvote += 1
            #cur.execute("INSERT INTO answers(upvote) VALUES(%d)",(uvote))
            #mysql.connection.commit()
            #cur.close()
    ups = 0
    if result > 0:
